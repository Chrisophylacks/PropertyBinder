        ...
        where TContext : class
    {
        ...
        public void To(Expression<Func<TContext, T>> targetExpression)
        {
            ...
            for (int i = 0; i < _clauses.Count; ++i)
            {
                ...
                _binder.AddRule(assignment, key, _clauses[i].Item3.CreateContext(typeof(TContext).Name, key), _runOnAttach, i == 0 && _canOverride, assignmentExpression, dependencies);
            }
        }
        ...
        public void To(Action<TContext, T> action)
        {
            ...
            for (int i = 0; i < _clauses.Count; ++i)
            {
                ...
                _binder.AddRule(ctx =>
                    {
                        invoke(ctx, action);
                    }, _key, _clauses[i].Item3.CreateContext(typeof(TContext).Name, _key), _runOnAttach, i == 0 && _canOverride, invokeExpression, new[] { invokeExpression }.Concat(_dependencies));
            }
        }
