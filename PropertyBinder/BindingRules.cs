...
{
    internal sealed class BindingRules<TContext>
        where TContext : class
    {
        ...
        private BindingRules(IBindingNodeRoot<TContext> rootNode, IDictionary<string, List<Action<TContext>>> keyedActions, UniqueActionCollection<TContext> attachActions)
        {
            _rootNode = rootNode;
        ...
        public BindingRules<TNewContext> Clone<TNewContext>()
            where TNewContext : class, TContext
        {
            return new BindingRules<TNewContext>(_rootNode.CloneRootForDerivedType<TNewContext>(), _keyedActions.ToDictionary(x => x.Key, x => new List<Action<TNewContext>>(x.Value)), _attachActions.Clone<TNewContext>());
        }
